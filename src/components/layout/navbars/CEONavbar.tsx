...
const navItems: NavItem[] = [
  ...
  { icon: Bot, label: 'AI Management', path: '/ceo/ai' },
  { icon: Shield, label: 'User Control', path: '/ceo/users' },
  { icon: Wallet, label: 'Finance', path: '/ceo/finance' },
...
const CEONavbar = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/ceo/team?search=${encodeURIComponent(searchQuery)}`);
    }
  };
    ...
    <nav className="sticky top-0 z-40 bg-white/70 dark:bg-black/40 backdrop-blur-2xl backdrop-saturate-150 border-b border-white/30 dark:border-white/10 shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
      <div className="flex items-center justify-between h-16 px-4 md:px-6">
        ...
        <div className="hidden md:flex items-center">
          <span className="text-lg font-display font-bold text-foreground">CEO Hub</span>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 bg-white/90 dark:bg-black/80 backdrop-blur-xl border-white/30 dark:border-white/10">
              ...
              <DropdownMenuItem onClick={() => navigate('/ceo/ai')} className="text-foreground focus:bg-white/50 dark:focus:bg-white/10">
