...
const BusinessDashboard = () => {
  ...
  return (
    <DashboardLayout>
      ...
      <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-4 md:mb-6">
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base md:text-lg font-display font-semibold text-foreground">Quick Stats</h2>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between p-2.5 md:p-3 bg-muted/30 rounded-lg">
              <span className="text-xs md:text-sm text-muted-foreground">Total Network</span>
            ...
            <div className="flex items-center justify-between p-2.5 md:p-3 bg-muted/30 rounded-lg">
              <span className="text-xs md:text-sm text-muted-foreground">Current Rank</span>
          ...
          {/* Upgrade CTA for non-premium - inside Quick Stats on mobile */}
          {!isPremium && (
            <div className="mt-4 p-3 bg-gradient-to-br from-accent/10 to-primary/10 rounded-lg border border-accent/20">
              <div className="flex items-center gap-2 mb-2">
                <Crown className="w-4 h-4 text-accent" />
                <span className="text-sm font-semibold text-foreground">Unlock Full Potential</span>
              </div>
              <p className="text-xs text-muted-foreground mb-3">
                Upgrade to earn commissions & track rank.
              </p>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <UserPlus className="w-4 h-4 md:w-5 md:h-5 text-success" />
              <h2 className="text-base md:text-lg font-display font-semibold text-foreground">Recent Signups</h2>
            </div>
          ...
          {loading ? (
            ...
            <div className="space-y-2 md:space-y-3">
              {recentReferralsList.map((referral, i) => (
                <div key={i} className="flex items-center justify-between p-2.5 md:p-3 bg-muted/30 rounded-lg">
                  <div className="flex items-center gap-2 md:gap-3">
                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-accent/10 flex items-center justify-center text-accent font-semibold text-sm md:text-base">
                      {referral.avatar}
                    </div>
                    <div>
                      <p className="font-medium text-foreground text-sm">{referral.name}</p>
                      <p className="text-[10px] md:text-xs text-muted-foreground">{referral.date}</p>
                    </div>
                  </div>
                  <Badge className={`text-[10px] md:text-xs ${referral.status === 'active' ? 'bg-success/10 text-success border-success/30' : 'bg-muted text-muted-foreground border-border'}`}>
            ...
            <div className="text-center py-6 md:py-8 text-muted-foreground">
              <UserPlus className="w-10 h-10 md:w-12 md:h-12 mx-auto mb-3 opacity-30" />
      ...
      {/* Rank Progress - THIRD ROW (full width) - Blurred for non-premium */}
      <PremiumBlur message="Upgrade to track rank progression & earn bonuses">
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base md:text-lg font-display font-semibold text-foreground">Rank Progress</h2>
          </div>
          
          {nextRank && (
            <div className="mb-4 p-3 md:p-4 rounded-lg bg-muted/30">
              ...
              <Progress value={referralProgress} className="h-2.5 md:h-3 mb-2" />
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
            {ranks.map((rank) => {
              ...
              return (
                <div 
                  key={rank.name} 
                  className={`flex flex-col items-center gap-2 p-2 md:p-3 rounded-lg border text-center ${
                ...
                >
                  ...
                  <div>
                    <p className={`text-xs md:text-sm font-medium ${isCurrent ? 'text-gold' : isAchieved ? 'text-success' : 'text-foreground'}`}>
                      {rank.name}
                    </p>
                    <p className="text-[9px] md:text-[10px] text-muted-foreground">{rank.min_referrals} refs</p>
                  </div>
                  {isAchieved && <CheckCircle className="w-3 h-3 text-success" />}
                </div>
              );
            })}
          </div>
